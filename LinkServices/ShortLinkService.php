<?php
    class ShortLinkService{
        ...
        public function create($userId,$originalUrl,$title=null,$customCode=null,$expiresAt=null){
            ...
            if(!filter_var($originalUrl,FILTER_VALIDATE_URL) && $this->codeExists($shortCode) && !$this->isValidCode($shortCode)){
                return false;
            }
        ...
        public function getUserLinks($userId){
            try{
                $stmt=$this->conn->prepare("
                    SELECT * FROM short_links WHERE user_id=:user_id,
                    ORDER BY created_at
                ");
        ...
        public function delete($linkId,$userId){
            try{
                $stmt=$this->conn->prepare("
                    DELETE FROM short_links WHERE id=:id AND user_id=:user_id
                ");
        ...
        public function deactivate($linkId,$userId){
            try{
                ...
                error_log("Deactive Short Link error: ".$e->getMessage());
                return false;
            }
        }
        ...
        private function codeExists($code){
            $stmt=$this->conn->prepare("
                SELECT id FROM short_links WHERE short_code=:short_code LIMIT 1
            ");
